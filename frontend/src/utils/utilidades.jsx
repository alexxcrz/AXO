...
export function normalizeInventoryMovementType(value) {
  ...
  if (["transfer_return", "devolucion", "retorno"].includes(key)) return INVENTORY_MOVEMENT_TRANSFER_RETURN;
  if (["transfer", "transferencia", "traslado"].includes(key)) return INVENTORY_MOVEMENT_TRANSFER;
  return INVENTORY_MOVEMENT_RESTOCK;
}

export function isOrderInventoryPrimaryWarehouse(warehouse, primaryWarehouse = ORDER_INVENTORY_PRIMARY_WAREHOUSE) {
  return normalizeKey(warehouse) === normalizeKey(primaryWarehouse);
}

export function collectOrderInventorySecondaryWarehouses(destinations = [], transferSummaries = [], primaryWarehouse = ORDER_INVENTORY_PRIMARY_WAREHOUSE) {
  const warehouses = new Set();
  const rememberWarehouse = (warehouse) => {
    const normalized = String(warehouse || "").trim();
    if (!normalized || isOrderInventoryPrimaryWarehouse(normalized, primaryWarehouse)) return;
    warehouses.add(normalized);
  };

  destinations.forEach((destination) => rememberWarehouse(destination?.warehouse));
  transferSummaries.forEach((item) => {
...
export function filterOrderInventoryItemsByWarehouse(items, warehouse, primaryWarehouse = ORDER_INVENTORY_PRIMARY_WAREHOUSE) {
  const normalizedWarehouse = String(warehouse || "").trim().toLowerCase();
  if (!normalizedWarehouse) return items;

  if (isOrderInventoryPrimaryWarehouse(warehouse, primaryWarehouse)) {
    return items.filter((item) => {
      const stockInAlmacen = Number(item.stockUnits || 0) > 0;
      const targets = item.transferTargets || [];
      if (!targets.length) return true;
      return stockInAlmacen;
    });
  }

  return items.filter((item) => (item.transferTargets || []).some((target) =>
    String(target.warehouse || "").trim().toLowerCase() === normalizedWarehouse
    && Number(target.availableUnits || 0) > 0,
  ));
}
...
export function ensureSelectOptionsIncludeValue(options, savedValue, groupName = "Registrado") {
  const list = Array.isArray(options) ? [...options] : [];
  const normalized = String(savedValue || "").trim();
  if (!normalized) return list;
  const exists = list.some((option) => String(option?.value || "").trim() === normalized);
  if (exists) return list;
  return [{ value: normalized, label: normalized, group: groupName }, ...list];
}
...
export function canAccessPage(user, pageId, permissions) {
  ...
  if (pageId === PAGE_RETAIL) {
    return userHasAnyRetailAreaScope(user, permissions);
  }
...
export function canDoAction(user, actionId, permissions) {
  return resolveCanDoAction(user, actionId, permissions);
}

const POST_LOGIN_PAGE_PRIORITY = [
  PAGE_CUSTOM_BOARDS,
  PAGE_TRANSPORT,
  PAGE_BIBLIOTECA,
  PAGE_DASHBOARD,
...
export function resolveFirstAccessiblePage(user, permissions) {
  if (!user) return PAGE_DASHBOARD;
  const allowed = NAV_ITEMS
    .filter((item) => item.id !== PAGE_DASHBOARD_BUILDER)
    .filter((item) => canAccessPage(user, item.id, permissions))
    .map((item) => item.id);
  if (!allowed.length) return PAGE_BIBLIOTECA;
  if (normalizeRole(user.role) === ROLE_JR) {
    return allowed.includes(PAGE_CUSTOM_BOARDS) ? PAGE_CUSTOM_BOARDS : allowed[0];
  }
  const preferred = POST_LOGIN_PAGE_PRIORITY.find((pageId) => allowed.includes(pageId));
  return preferred || allowed[0];
}

export function normalizeStoredActivePage(pageId) {
  const resolved = pageId && PAGE_ROUTE_ALIASES[pageId] ? PAGE_ROUTE_ALIASES[pageId] : pageId;
  if (!resolved || resolved === PAGE_DASHBOARD_BUILDER) return PAGE_DASHBOARD;
  if (resolved === PAGE_AREA_SHELL) return PAGE_AREA_SHELL;
  return resolved;
}
...
export function canCreateBoardRowRecord(user, board, permissions) {
  if (!user || !board) return false;
  return canDoBoardActionForUser(user, board, "createBoardRow", permissions);
}
